      ...
      {isOpen ? (
        <form onSubmit={handleSearch} className="relative flex items-center justify-center w-full">
          <div className="relative w-1/2">
            <input
              type="text"
              placeholder="Search for indoor or outdoor plants..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="bg-green-50 px-4 py-2 rounded-lg focus:outline-none w-full placeholder:text-green-700 text-green-800"
            />
            <button
              type="submit"
        ...
        <button onClick={() => setIsOpen(true)} className="text-green-700 hover:text-green-900">
          <HiMagnifyingGlass className="h-6 w-6" />
        </button>
      )}
    </div>
